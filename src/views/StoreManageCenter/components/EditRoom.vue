...
onMounted(async () => {
  ...
  const response = await fetch(`http://iorlvm.i234.me:8080/rooms/${roomId}`)
  room.value = await response.json()
  photoUrl.value = `http://iorlvm.i234.me:8080/photos/${room.value.photoId}`
})
...
const updateRoom = async () => {
  ...
  const response = await fetch(`http://iorlvm.i234.me:8080/rooms/update/${room.value.id}`, {
    method: 'PUT',
    body: formData,
  })
